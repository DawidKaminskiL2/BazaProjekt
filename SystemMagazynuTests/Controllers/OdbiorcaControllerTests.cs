...
namespace SystemMagazynu.Tests.Controllers
{
    public class OdbiorcaControllerTests
    {
        ...
        public async Task PutOdbiorca_InvalidId_ReturnsBadRequest()
        {
            ...
            // Act
            var result = await controller.PutOdbiorca(999, odbiorca); 

            // Assert
            var badRequest = Assert.IsType<BadRequestObjectResult>(result);
            Assert.Equal("ID w ścieżce nie zgadza się z ID w obiekcie", badRequest.Value);
        }
        ...
        public async Task DeleteOdbiorca_Valid_ReturnsNoContent()
        {
            ...
            var odbiorca = new Odbiorca { Nazwa = "Do usunięcia", Email = "test@test.pl", Telefon = "123", Adres = "Adres", CzyAktywny = true };
            context.Odbiorcy.Add(odbiorca);
            await context.SaveChangesAsync();
